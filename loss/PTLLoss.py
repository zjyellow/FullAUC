...
class PTLLoss(nn.CrossEntropyLoss):
    def __init__(self, **options):
        super(PTLLoss, self).__init__()
    ...
    def forward(self, x, y, labels=None, train_mode=True):
        embedding, logits, labels = x, y, labels
...
################################################
    ...
    for c in range(num_classes):
        ...
        if torch.isnan(triplet_loss).any():
            ...
            total_local += 1
        # print(Ftriplet_loss_local(anchors_local, positives_local, negatives_local))
        # print(triplet_loss_local)
        # print(total_local)
    ...
    for c in range(num_classes):
        ...
        if torch.isnan(triplet_loss).any():
            ...
            total_global += 1
            print(triplet_loss_global)
            print(total_global)
        # triplet_loss_global += Ftriplet_loss_global(anchors_global, positives_global, negatives_global)
        # total_global += 1
