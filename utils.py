...
def save_networks(networks, result_dir, name='', loss='', criterion=None, epoch=0):
    mkdir_if_missing(osp.join(result_dir, 'checkpoints'))
    weights = networks.state_dict()
    filename = '{}/checkpoints/{}_{}_epoch={}.pth'.format(result_dir, name, loss, epoch)
    torch.save(weights, filename)
    if criterion:
        weights = criterion.state_dict()
        filename = '{}/checkpoints/{}_{}_criterion_epoch={}.pth'.format(result_dir, name, loss, epoch)
        torch.save(weights, filename)

def save_networks2(networks, result_dir, name='', loss='', criterion=None, epoch=0): #net, model_path, file_name, '', criterion
    mkdir_if_missing(osp.join(result_dir, 'checkpoints'))
    weights = networks.state_dict()
    filename = '{}/checkpoints/{}_{}_epoch={}.pth'.format(result_dir, name, loss, epoch) #mnist/checkpoints/classifier32_MyLoss_new_0_False_
    torch.save(weights, filename)
    if criterion: # True
        weights = criterion.state_dict()
        filename = '{}/checkpoints/{}_{}_criterion_epoch={}.pth'.format(result_dir, name, loss, epoch)
