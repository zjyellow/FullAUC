    ...
    for batch_idx, (data, labels) in enumerate(trainloader):
        ...
        with torch.set_grad_enabled(True):
            ...
            if options['loss'] == 'OpenAUCLoss':
                if options['model'] == 'resnet34':
                    logits, loss = criterion(y, labels, net.module.linear, x)
                else:
                    logits, loss = criterion(y, labels, net.module.fc, x)
            elif options['loss'] == 'Proxy_Anchor':
                logits, loss = criterion(x, y, labels)
            elif options['loss'] in ['MDL4OW']:
                logits, loss = criterion(ori_x, re_x, logits, labels)
            else:
                logits, loss = criterion(x, y, labels)
            
            loss.backward()

            # torch.nn.utils.clip_grad_norm_(net.parameters(), max_norm=2.0, norm_type=2.0)
            # torch.nn.utils.clip_grad_norm_(criterion.parameters(), max_norm=2.0, norm_type=2.0)
